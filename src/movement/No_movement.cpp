...
void No_movement::determine_z_paws_position_not_get_up(Side &side, int sequence_number, double a, double b)
{
	m_paw_position.front[coord_z]  = a*(m_paw_position.front[coord_x] + HALF_LENGTH) + b;
	m_paw_position.middle[coord_z] = a*m_paw_position.middle[coord_x] + b;
	m_paw_position.back[coord_z]   = a*(m_paw_position.back[coord_x] - HALF_LENGTH) + b;
	...
	{
		if(side.get_side_id() == side_left)
			m_paw_position.front[coord_z]  = just_get_down_paw(a*(side.get_front_paw().m_x_center + m_distance / 2 + HALF_LENGTH) + b, side.get_front_paw(), NO_MOVEMENT_STEP_DIST);
		else
			m_paw_position.back[coord_z]   = just_get_down_paw(a*(side.get_back_paw().m_x_center + m_distance / 2 - HALF_LENGTH) + b, side.get_back_paw(), NO_MOVEMENT_STEP_DIST);
	}
	else if(sequence_number == 1)
	{
		if(side.get_side_id() == side_left)
			m_paw_position.middle[coord_z] = just_get_down_paw(a*(side.get_middle_paw().m_x_center + m_distance / 2) + b, side.get_middle_paw(), NO_MOVEMENT_STEP_DIST);
		else
			m_paw_position.middle[coord_z] = just_get_down_paw(a*(side.get_middle_paw().m_x_center + m_distance / 2) + b, side.get_middle_paw(), NO_MOVEMENT_STEP_DIST);
	}
	else if(sequence_number == 2)
...
bool No_movement::test_for_good_position_z(Side &side,double a, double b, int sequence_number)
{
	//test if one or more paw is not at the correct x,y or z position
	if(sequence_number == 0)
	{
		if(side.get_side_id() == side_left)
			return((a*(side.get_front_paw().m_x_center + m_distance / 2 + HALF_LENGTH) + b) == (side.get_front_paw().m_current_coords.z));
		else
			return((a*(side.get_back_paw().m_x_center + m_distance / 2 - HALF_LENGTH) + b) == ( side.get_back_paw().m_current_coords.z));
	}
	else if(sequence_number == 1)
	{
		if(side.get_side_id() == side_left)
			return((a*(side.get_middle_paw().m_x_center + m_distance / 2) + b) == ( side.get_middle_paw().m_current_coords.z));
		else
			return((a*(side.get_middle_paw().m_x_center + m_distance / 2) + b) == ( side.get_middle_paw().m_current_coords.z));
	}
	else if(sequence_number == 2)
...
Paw_position No_movement::determine_paws_position(Side &side, int sequence_number, double a, double b, double paw_spreading)
{
	...
	else
	{
		determine_x_paws_position(side, sequence_number);
		determine_y_paws_position(side, sequence_number, paw_spreading);
		determine_z_paws_position_not_get_up(side, sequence_number, a, b);
	}
