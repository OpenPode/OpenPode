...
{
	m_paw_position.front[coord_x]  = side.get_paws_position().front_paw.x  - cos(m_angle) * m_step_distance_x * m_current_step_number; // * m_current_step_number
	m_paw_position.middle[coord_x] = side.get_paws_position().middle_paw.x - cos(m_angle) * m_step_distance_x * m_current_step_number;
	m_paw_position.back[coord_x]   = side.get_paws_position().back_paw.x   - cos(m_angle) * m_step_distance_x * m_current_step_number;
	//cout << "cos " << cos(m_angle) << " dist  " << m_step_distance_x << " dist_plus " << cos(m_angle) * m_step_distance_x << " result " << m_paw_position.front[coord_x] << endl;
	if(side.get_side_id() == side_left)
	{
		if(sequence_number == 0)
			m_paw_position.front[coord_x]  = goto_position(side.get_front_paw().m_current_coords.x, side.get_front_paw().m_x_center + cos(m_angle) * m_distance / 2, m_step_number - m_current_step_number);
		else if(sequence_number == 1)
			m_paw_position.middle[coord_x] = goto_position(side.get_middle_paw().m_current_coords.x, side.get_middle_paw().m_x_center + cos(m_angle) * m_distance / 2, m_step_number - m_current_step_number);
		else
			m_paw_position.back[coord_x]   = goto_position(side.get_back_paw().m_current_coords.x, side.get_back_paw().m_x_center + cos(m_angle) * m_distance / 2, m_step_number - m_current_step_number);
	}
	else
	{
		if(sequence_number == 0)
	...
	//cout << "x " << m_paw_position.front[coord_x] << endl;
}

void complete_linear_movement::determine_y_paws_position(Side &side, int sequence_number, double paw_spreading)
{
	m_paw_position.front[coord_y]  = side.get_front_paw().m_current_coords.y -  sin(m_angle) * m_step_distance_y; // * m_current_step_number
	...
	//cout << "sin " << sin(m_angle) << " dist  " << m_step_distance_y << " dist_plus " << sin(m_angle) * m_step_distance_y << " result " << m_paw_position.front[coord_y] << endl;
	if(sequence_number == 0)
	{
		if(side.get_side_id() == side_left)
			m_paw_position.front[coord_y]  = goto_position(side.get_front_paw().m_current_coords.y, side.get_side_coef() * (paw_spreading + abs(sin(m_angle)) * m_distance / 2), m_step_number - m_current_step_number);
		else
			m_paw_position.back[coord_y]   = goto_position(side.get_back_paw().m_current_coords.y, side.get_side_coef() * (paw_spreading + abs(sin(m_angle)) * m_distance / 2), m_step_number - m_current_step_number);
	}
	else if(sequence_number == 1)
	{
		if(side.get_side_id() == side_left)
			m_paw_position.middle[coord_y] = goto_position(side.get_middle_paw().m_current_coords.y, side.get_side_coef() * (paw_spreading + abs(sin(m_angle)) * m_distance / 2), m_step_number - m_current_step_number);
		else
			m_paw_position.middle[coord_y] = goto_position(side.get_middle_paw().m_current_coords.y, side.get_side_coef() * (paw_spreading + abs(sin(m_angle)) * m_distance / 2), m_step_number - m_current_step_number);
	}
	else
	...
	//cout << " y " << m_paw_position.front[coord_y] << endl;
...
void complete_linear_movement::determine_z_paws_position(Side &side, int sequence_number, double front_height, double back_height)
{
	...
	{
		if(side.get_side_id() == side_left)
			m_paw_position.front[coord_z]  = get_up_paw(front_height*(side.get_front_paw().m_x_center + cos(m_angle) * m_distance / 2 + HALF_LENGTH) + back_height, side.get_front_paw(), m_step_distance_z);
		else
			m_paw_position.back[coord_z]   = get_up_paw(front_height*(side.get_back_paw().m_x_center + cos(m_angle) * m_distance / 2 - HALF_LENGTH) + back_height, side.get_back_paw(), m_step_distance_z);
	}
	else if(sequence_number == 1)
	{
		if(side.get_side_id() == side_left)
			m_paw_position.middle[coord_z] = get_up_paw(front_height*(side.get_middle_paw().m_x_center + cos(m_angle) * m_distance / 2) + back_height, side.get_middle_paw(), m_step_distance_z);
		else
			m_paw_position.middle[coord_z] = get_up_paw(front_height*(side.get_middle_paw().m_x_center + cos(m_angle) * m_distance / 2) + back_height, side.get_middle_paw(), m_step_distance_z);
	}
	else
